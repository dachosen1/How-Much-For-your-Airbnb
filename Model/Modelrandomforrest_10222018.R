...
scoringData2 <- select(scoringData,neighbourhood_group_cleansed,room_type,accommodates,bathrooms,bedrooms,
                     cleaning_fee,availability_30,review_scores_communication,reviews_per_month)

pred2 <- predict(model1,newdata=scoringData2)
...
write.csv(submissionFile, 'sample_submission4.csv',row.names = F)
