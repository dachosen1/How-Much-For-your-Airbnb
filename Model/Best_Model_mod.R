...
tune_grid <- expand.grid(
  ...
  eta = seq(from =0.002, to = 0.01,by = .002),
...
pred_test_2 <- predict(xgb_model_2, newdata = testset)
sqrt(mean((pred_test_2 - testset$price)^2))
...
pred_gbm_scoring <- predict(xgb_model_2, newdata = scoringData)
...
write.csv(submissionFile, 'sample_submission26.csv',row.names = F)
