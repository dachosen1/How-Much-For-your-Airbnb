...
dataset <- read.csv("scoringData.csv")
dataset2 <- select(id,host_id,zipcode,room_type,property_type,accommodates,host_since,host_response_time,
                   bathrooms,bedrooms,beds,bed_type,neighbourhood_cleansed,neighbourhood_group_cleansed,
...
dataset2$zipcode[is.na(dataset2$zipcode)] <- zip
...
dataset2$property_type[!dataset2$property_type == "Apartment" & !dataset2$property_type =="House" &
                        !dataset2$property_type =="Loft" & !dataset2$property_type =="Condominium"] <- "Other"
...
#reviews per month 
dataset2$reviews_per_month[is.na(dataset2$reviews_per_month)] <- median(dataset2$reviews_per_month, na.rm = TRUE)

#last_review 
dataset2$last_review[is.na(dataset2$last_review)] <- median(dataset2$last_review, na.rm = TRUE)

#first_review
dataset2$first_review[is.na(dataset2$first_review)] <- median(dataset2$first_review, na.rm = TRUE)

#-------------------------------------------Data Transformation 
week <- select(dataset2, - amenities,-zipcode, -monthly_price)
...
#-----------------------------------------monthly price
month <- select(dataset2, - amenities,-zipcode, -weekly_price)
...
write.csv(dataset3, 'cleandata(test).csv',row.names = F)
